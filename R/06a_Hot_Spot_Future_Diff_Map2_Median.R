...
# get an median from the layers that do have data
map2_median <- app(map2_stack, fun = median, na.rm = TRUE)

names(map2_median) <- "map2_future_diff_median"

################################################################################
# SECTION 4: Write Output
################################################################################

out_path <- file.path(hot_spot_dir, "tbcmp_hot_spot_map2_future_diff_median.tif")
...
cat(sprintf("Saved median map2 raster -> %s\n", out_path))
