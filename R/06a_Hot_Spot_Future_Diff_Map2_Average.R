...
map2_mean <- app(map2_stack, fun = mean, na.rm = TRUE)

names(map2_mean) <- "map2_future_diff_mean"

################################################################################
# SECTION 4: Write Output
################################################################################

out_path <- file.path(hot_spot_dir, "tbcmp_hot_spot_map2_future_diff_mean.tif")
...
cat(sprintf("Saved averaged map2 raster -> %s\n", out_path))
