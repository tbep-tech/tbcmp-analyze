...
counties <- vect(tbcmp_cnt)  # Adjust path if needed (e.g., "./data/tbcmp_cnt.shp")
...
if (!same.crs(counties, raster_time1)) {
...
lc_sabre = vmeasure_calc(raster_time1, raster_time2)
lc_sabre
...
r_kappa <- raster.change(raster_time1, raster_time2, stat = "kappa")
plot(r_kappa, main="Kappa Statistic for Change")
...
#Partition raster to test
hernando <- tbcmp_cnt |>
            filter(county %in% "Hernando") |>
            vect()

county_buffer <- buffer(hernando, width = 100)  # 100m buffer

# Crop change raster to buffered county
change_county <- crop(change_raster, county_buffer, mask = TRUE)

# Check if there are any changes in this county
total_cells <- sum(!is.na(values(change_county)))
changed_cells <- sum(values(change_county), na.rm = TRUE)

cat(sprintf("  Total cells: %d\n", total_cells))
cat(sprintf("  Changed cells: %d\n", changed_cells))
...
change_points <- as.points(change_county)
